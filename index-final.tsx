...
// Ganti URL dan Anon Key sesuai dengan Project Settings > API di Supabase Anda
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Monitor perubahan status login secara real-time
    const { data: authListener } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      if (session) setCurrentView('admin');
      else setCurrentView('identity');
    });
  ...
  const handleBooking = async () => {
    ...
    if (!error) {
        ...
        // Redirect ke nomor WhatsApp Customer/Owner (Menggunakan 62)
    ...
    <SafeAreaView style={styles.containerBlue}>
      ...
      <FlatList data={SERVICES} contentContainerStyle={{padding: 20}} renderItem={({item}) => (
        <TouchableOpacity style={styles.cardService} onPress={() => {setSelectedService(item); setCurrentView('booking');}}>
          <MaterialCommunityIcons name={item.icon as any} size={28} color="#1e3a8a" />
...
const styles = StyleSheet.create({
  ...
  btnText: { color: '#1e3a8a', fontWeight: 'bold', fontSize: 16 },
